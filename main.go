...
import (
	...
	"github.com/pocketbase/pocketbase/models" // <--- FIXED: Added missing import
...
func main() {
	...
	// CORRECT HOOK: app.OnBeforeServe().BindFunc() is the modern replacement for .Add()
	...
	// CORRECT HOOK: app.OnModelAfterUpdate("collection") is the modern low-level replacement
	// for the old app.OnRecordAfterUpdate("collection")
	app.OnModelAfterUpdate("orders").BindFunc(func(e *core.ModelEvent) error {
		// Cast the generic model to a Record, which is expected for collection events
		...
		// Use OriginalCopy() to access the data before the update
